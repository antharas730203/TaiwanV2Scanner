...
class MainActivity : Activity() {
    ...
    private fun buildDrawer(): LinearLayout {
        ...
        top.addView(roundButton("×", 48, false).apply { setOnClickListener { toggleDrawer(false) } }, LinearLayout.LayoutParams(dp(48), dp(48)))
        panel.addView(top)

        panel.addView(sectionHeader("GitHub 設定"))
        ...
        attachExpandable(panel, 1, githubBox)

        panel.addView(sectionHeader("自動排程"))
        ...
        scheduleBox.addView(TextView(this).apply { text = "排程時間"; textSize = 14f; setTextColor(Color.LTGRAY); setPadding(0, dp(10), 0, dp(4)) })
        ...
        attachExpandable(panel, 2, scheduleBox)

        panel.addView(sectionHeader("關於"))
        val aboutBox = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; visibility = View.GONE; setPadding(0, dp(4), 0, dp(8)) }
        aboutBox.addView(TextView(this).apply { text = "V0.8.2"; textSize = 18f; setTextColor(Color.WHITE) })
        aboutBox.addView(TextView(this).apply { text = "台股 V2 掃描器\n完整 TWSE＋TPEX 掃描\n第一層轉機／動能預篩\nExact Alarm 排程與 GitHub JSON 歸檔"; textSize = 14f; setTextColor(Color.LTGRAY); setPadding(0, dp(6), 0, 0) })
        panel.addView(aboutBox)
        attachExpandable(panel, 3, aboutBox)
        return panel
    }

    private fun sectionHeader(textValue: String): TextView = TextView(this).apply {
        text = "▶  $textValue"
        textSize = 17f
        setTextColor(Color.WHITE)
    ...
    private fun attachExpandable(panel: LinearLayout, index: Int, content: View) {
        val header = panel.getChildAt(panel.childCount - 2) as TextView
        header.setOnClickListener {
            content.visibility = if (content.visibility == View.VISIBLE) View.GONE else View.VISIBLE
            header.text = if (content.visibility == View.VISIBLE) "▼  ${header.text.substring(3)}" else "▶  ${header.text.substring(3)}"
        }
    }
    ...
    private fun runFullScan(scanButton: View) {
        ...
        Thread {
            ...
            runOnUiThread { result.text = report; status.text = "手動掃描＋第一層完成"; scanButton.isEnabled = true; refreshStatus() }
        }.start()
    }
    ...
    private fun verifyGitHubButton(button: View) {
        ...
        Thread {
            ...
            runOnUiThread { status.text = report.lineSequence().firstOrNull() ?: report; result.text = report; button.isEnabled = true }
        }.start()
    }
    ...
    private fun uploadLatestJson(button: View) {
        ...
        val baseDir = getExternalFilesDir(null) ?: filesDir; val layer1File = File(baseDir, "layer1_latest.json")
        if (!layer1File.exists()) { Toast.makeText(this, "找不到最新第一層 JSON，請先重新掃描", Toast.LENGTH_LONG).show(); return }
        ...
        Thread {
            ...
            runOnUiThread { result.text = report; status.text = if (report.startsWith("成功")) "最新 JSON 上傳完成" else "最新 JSON 上傳失敗"; button.isEnabled = true }
        }.start()
    }
    ...
    private fun refreshStatus() {
        ...
        val token = if (GitHubTokenStore.hasToken(this)) "已設定" else "未設定"; val times = prefs.getString("schedule_times", ScanScheduler.DEFAULT_TIMES) ?: ScanScheduler.DEFAULT_TIMES; val diag = getSharedPreferences("diagnostics", 0); val l1 = diag.getString("layer1_status", "尚未執行") ?: "尚未執行"; val engine = diag.getString("schedule_engine", "EXACT_ALARM") ?: "EXACT_ALARM"
